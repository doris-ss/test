...
def build_tree_from_hierarchy(label_hierarchy):
    """
    从给定的标签层级结构构建树结构。

    :param label_hierarchy: 代表标签层级的字典。
    :return: 树的根节点。
    """
...
#save parent/child to file
def print_parent_child_pairs(node,file,parent = None):
    #for node, children in tree.items():
        if parent is not None:
            file.write(f"Parents({parent.name}) Child({node.name})\n")
        for child in node.children:
                print_parent_child_pairs(child,file,node)

def print_parentchild(node,file,parent = None):
    #for node, children in tree.items():
        if parent is not None:
            file.write(f"ParentChild({parent.name},{node.name})\n")
        for child in node.children:
                print_parentchild(child,file,node)

if __name__ == '__main__':
    get_data_from_meta()
    get_hierarchy()
    split_train_dev_test()
    tree = build_tree_from_hierarchy(get_hierarchy())
    # with open('parent_child_relationship.txt','w')as file:
    #     print_parent_child_pairs(tree,file)
    # with open('fact.txt', 'w') as file:
    #     print_parentchild(tree, file)
